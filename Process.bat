...
IF %stage_num%==7 (
	%matlabpath_2019b% -wait -log -nodesktop -nosplash -r "cd(getenv('funPath'));mf=getenv('mfilename'); mfile_name=strtrim(mf); ip=getenv('mname');model_file=strtrim(ip);ret=TestCasesGenSLDV(model_file,mfile_name);pause(5);exit"
	goto return_value
)
IF %stage_num%==8 (
